    ...
    <div className="hosted-topic-editor stack">
      ...
      <label className="field hosted-field">
        {value.type === "proposition"
          ...
          : "Question or case"}
        <textarea
          aria-label={value.type === "proposition"
            ? "Topic claim or question"
            : "Topic question or case"}
          dir="auto"
          rows={2}
          ...
          placeholder={value.type === "proposition"
      ...
      <div className="hosted-position-grid">
        {(["positionA", "positionB"] as const).map((key, index) => (
          <div className="hosted-position-card" key={key}>
            <h3>
              {value.type === "proposition"
                ? index === 0 ? "FOR position" : "AGAINST position"
                : `Position ${index === 0 ? "A" : "B"}`}
            </h3>
            <label className="field hosted-field">
              {`${index === 0 ? "Position A" : "Position B"} text`}
              <textarea
                aria-label={`${index === 0 ? "Position A" : "Position B"} text`}
                dir="auto"
                rows={3}
                maxLength={HOSTED_CONFIG.MAX_POSITION_CHARS}
                value={value[key].text}
                onChange={(event) =>
                  setPosition(key, "text", event.target.value)}
            ...
            <label className="field hosted-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setPosition(key, "attribution", event.target.value)}
      ...
      <div className="hosted-sources-head">
        <div>
          ...
          <p>
            Paste each source separately. Participants on both sides will see
            them.
          </p>
        </div>
      ...
      {value.sources.length === 0
        ? (
          <p className="hosted-empty">
            No sources added. Source use will not be scored.
          </p>
        )
        : (
          value.sources.map((source, index) => (
            <div className="hosted-source-editor" key={source.id}>
              <div className="hosted-source-editor__head">
                ...
                <button
                  ...
                  onClick={() =>
                    onChange({
                      ...value,
                      sources: value.sources.filter((_, sourceIndex) =>
                        sourceIndex !== index
                      ),
                    })}
                >
              ...
              <label className="field hosted-field">
                {`Source ${index + 1} label, optional`}
                <input
                  aria-label={`Source ${index + 1} label, optional`}
                  dir="auto"
                  maxLength={HOSTED_CONFIG.MAX_SOURCE_LABEL_CHARS}
                  value={source.label ?? ""}
                  onChange={(event) =>
                    setSource(index, { label: event.target.value || null })}
              ...
              <label className="field hosted-field">
                {`Source ${index + 1} text`}
                <textarea
                  aria-label={`Source ${index + 1} text`}
                  dir="auto"
                  rows={5}
                  maxLength={HOSTED_CONFIG.MAX_SOURCE_CHARS}
                  value={source.text}
                  onChange={(event) =>
                    setSource(index, { text: event.target.value })}
                />
              </label>
            </div>
          ))
