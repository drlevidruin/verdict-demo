...
export function Invite({ secure, code, busy, error, onJoin, onDismiss }: Props) {
    ...
    <div className="app invite-shell">
      ...
      <main className="center">
        <form className="card invite-card stack" onSubmit={(event) => {
          ...
          if (!busy && name.trim()) onJoin(name.trim());
        }}>
          <div>
            <p className="prompt-kicker">A friend invited you</p>
            <h1>Join their game</h1>
            <p className="muted">You&rsquo;ll get opposite sides of a debate and 60 seconds to write, then explore a fixed, prewritten sample verdict.</p>
          ...
          <p className="play-entry__hint">The timer starts after you both tap Ready.</p>
        </form>
